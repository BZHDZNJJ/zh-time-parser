...
中文时间表达的多样性远超出「正则匹配几个日期格式」能覆盖的范围：

- **口语化**：「上个月1号到今天」「5号到15号」「月初到今天」——这些表达不带年份、省略月份，需要基于当前日期做日历推算
- **缩写与习惯**：「Q1」「H2」「25年」「1-3月」——每个中文使用者都懂，但没有现成的解析规则
- **同比/环比**：经营分析中随处可见「本月同比」「Q2环比」，它们需要的是**两个**日期区间——本期和对比期，不是一个区间

做数据分析、BI 报表、AI 对话产品时，总有一个绕不开的环节：**用户说了一句中文时间，系统要知道具体是哪段日期。** 比如：

> 「帮我查一下上个月的数据」
> 「Q2同比情况怎么样」

如果你用正则硬写，很快就陷入成百上千条规则的泥潭。而如果用 `dateparser` 这类英文日期库，它对 `"上个月"` `"去年同期"` 这类中文表达完全无能为力。

所以就有了 `zh-time-parser`：一个**专注中文时间表达、零依赖、即装即用**的解析库。
