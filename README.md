...
中文自然语言时间语义解析器：将自然语言中的中文时间表达解析为结构化的时间语义，
可用于 Agent、BI、NL2SQL、搜索与工作流系统。

```python
"上个月"        → DateRange(2026-07-01, 2026-07-31)
"昨天中午"      → DateTimePoint(2026-08-10 12:00:00)
"最近一次"      → TemporalSelector(order="latest", limit=1)
"三天以内"      → TemporalBoundary(operator="<=", duration=3天)
"最近几天"      → AmbiguousTemporal(direction="recent", value=None)
"Q2同比"        → ComparisonRange(current=Q2, comparison=去年Q2)
```
...
| `dateparser` 等英文日期库 | 对中文表达完全无能为力。`"上个月"` `"去年同期"` 这类基础中文时间词都识别不了，更不用说同比/环比、口语号段或中文数字 |
...
## 核心设计：Temporal Semantic Layer

zh-time-parser 不是把中文直接翻译成日期，而是位于自然语言与下游系统之间的一层**时间语义表示**。

```
用户语言：  "最近一次"    "三天以内"    "最近几天"    "上个月"
              │              │             │            │
zh-time-parser│              │             │            │
              ▼              ▼             ▼            ▼
时间语义：  order=latest   operator=<="  direction=   DateRange
           limit=1        duration=3天   recent       start/end
                                        value=None
              │              │             │            │
              ▼              ▼             ▼            ▼
下游系统：  ORDER BY       WHERE ts      "您是指最近    SELECT ...
           ... LIMIT 1     <= NOW()+3    一周还是
                                         一个月？"
```
...
- **不替下游做决定**：对于「最近一次」，库不知道用户查的是什么数据，因此输出排序/选择语义（`order="latest", limit=1`），由下游系统自己映射为 `ORDER BY ... LIMIT 1`
- **不掩盖信息不足**：对于「最近几天」，库不猜测是 2 天、3 天还是 7 天，返回模糊意图让上层追问或应用默认策略
- **不只输出日期**：除了 `DateRange`，还输出比较运算符（`<` `<=` `>` `>=`）、事件排序、评估时点锚点等语义，直接对应 SQL、API 参数或搜索条件

因此，同一个「截至月底」，数据查询系统可以把它解释为 `WHERE date <= '2026-08-31'`，任务系统可以把它解释为 deadline 检查，SLA 系统可以把它解释为剩余天数计算。库只提供结构化语义，不绑定任何一种解释。
...
**Agent / 智能助手**
用户说「帮我查上个月的数据」，Agent 需要把「上个月」变成 `2026-07-01 ~ 2026-07-31` 才能调 API 或写 SQL。
...
**工作流 / 自动化任务**
任务描述「截至明天下午 3 点未处理的工单」，用 `TemporalBoundary` 直接映射为 `<` / `<=` 查询条件。
